...
export class ApiGatewayConstruct extends cdk.Construct {
    ...
    private addApiResourcesAndMethods() {
        const baseResource = this.restApi.root.addResource('entityManagement').addResource('v1');
        this.addGetEntityDetailsIntegration(baseResource);
    }

    private addGetEntityDetailsIntegration(baseResource: apigw.Resource) {
        const resource = baseResource.addResource('details').addResource('{entityid}');

        resource.addMethod(
            'GET',
            new apigw.LambdaIntegration(this.props.lambdaFunctions.getEntityDetails, {
    ...
    private createApiRole() {
        ...
        this.apiRole.addToPolicy(
            new iam.PolicyStatement({
                resources: [this.props.lambdaFunctions.getEntityDetails.functionArn],
