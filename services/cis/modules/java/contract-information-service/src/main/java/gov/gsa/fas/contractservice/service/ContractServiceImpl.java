...
public class ContractServiceImpl implements ContractService {
	...
	public RequestWrapper getListContractResponse(RequestWrapper inputStream) {
		try {
			...
			return new RequestWrapper(ContractServiceUtil.marshallException(ContractConstants.FAULT_CODE, ex.getMessage()));
		}
	}
	...
	private ListContractsType getListContracts(String entityID) throws ApplicationException {
		...
		try {
			List<String> internals = contractServiceDAO.getInternalContractNumber(ContractInternalIDType.DUNS.get(entityID));
			...
			if (internals != null && internals.size() > 0) {
				...
				for(String internal: internals) {
					...
					Optional<ContractsType> contract = getContractsType(
							gson.fromJson(masterDetails, ContractDataMaster.class), 
							gson.fromJson(addressDetails, Address.class));
	...
	private CSDetailPO validateGetContractData(CSDetailPO contractDetail, PORecordsType inPOLines) {
		...
		try {
			...
			if (!StringUtils.isBlank(contractMaster.getD402_dt_terminated())
	...
	private Optional<ContractsType> getContractsType(ContractDataMaster master, Address address) {
		String[] currentDate = DateUtil.getDateTime();
		...
		try {
			...
			if (StringUtils.isNotBlank(contractBeginDate) && DateUtil.dateCompare(currentDate[0], DateUtil.julianToGregf2(contractBeginDate)) == 2) {
				includeContract = false;
			}
			
			String contractEndDate = master.getD402_cont_end_dt();
			if (StringUtils.isNotBlank(contractEndDate) && DateUtil.dateCompare(currentDate[0], DateUtil.julianToGregf2(contractEndDate)) == 1) {
				includeContract = false;
			}  
			
			String contractTermDate = master.getD402_dt_terminated();
			if (StringUtils.isNotBlank(contractTermDate) && DateUtil.dateCompare(currentDate[0], DateUtil.julianToGregf2(contractTermDate)) == 1) {
				includeContract = false;
			}
			
			contractsType.setContractBeginDate(DateUtil.stringToXMLGregorianCalendar(DateUtil.julianToGregf2(contractBeginDate)));
			contractsType.setContractEndDate(DateUtil.stringToXMLGregorianCalendar(DateUtil.julianToGregf2(contractEndDate)));
			contractsType.setContractEndDate(DateUtil.stringToXMLGregorianCalendar(DateUtil.julianToGregf2(contractTermDate)));
			...
			if(StringUtils.isNotBlank(address.getD410_mail_adrs_pc())) {
				...
				if (wadrs3 != null && wadrs3.trim().length() > 0) {
					contractorAddress.append(wadrs1.substring(0, 32) + 
											 wadrs2.substring(0, 32) + 
											 wadrs3.substring(0, 32) + 
											 wcity + " " + wstate + " " + wzip);
